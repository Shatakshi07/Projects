...
import torch
import torch.nn as nn
from core.models import *  # Assuming your models are in core.models

# Set CUDA environment variables
os.environ['CUDA_DEVICE_ORDER'] = "PCI_BUS_ID"
os.environ['CUDA_VISIBLE_DEVICES'] = "0"  # Assuming you want to use GPU 0, but it's not available, so it's just for compatibility
...
# If multiple GPUs are available (this will always be False without CUDA)
...
if not args.use_gpu:
    ...
    print('Warning: GPUs are not available. Running on CPU.')

# Move model to CPU (no need for .cuda())
model.to('cpu')

# Print confirmation
print('------- success use CPU --------')
...
model_dir = os.path.join(RootDir, 'models', '%s_%s' % (model_name, args.my_description))
log_dir = os.path.join(RootDir, 'logs', '%s_%s.txt' % (model_name, args.my_description))
