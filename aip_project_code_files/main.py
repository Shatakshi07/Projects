...
import argparse
import time
from core.utils_original import calculate_Accuracy, get_img_list, get_model, get_data
from pylab import *
import random
...
if not os.path.exists(r'%s/aip_project_code_files/models/%s_%s' % (RootDir, model_name, args.my_description)):
    os.mkdir(r'%s/aip_project_code_files/models/%s_%s' % (RootDir, model_name, args.my_description))

with open(r'%s/aip_project_code_files/logs/%s_%s.txt' % (RootDir, model_name, args.my_description), 'w+') as f:
...
for epoch in range(args.epochs):
    ...
    for i, (start, end) in enumerate(zip(range(0, len(img_list), args.batch_size),
        ...
        # Inside MAIN2.PY before calling the model.forward()
        #print("Input shapes - img:", img.shape, "imageGreys:", imageGreys.shape)
        ...
        out, side_5, side_6, side_7, side_8 = model(img, imageGreys)
                # You may also want to print the shapes of tensors after the model.forward() call
        #print("Output shapes - out:", out.shape, "side_5:", side_5.shape, "side_6:", side_6.shape, "side_7:", side_7.shape, "side_8:", side_8.shape)
