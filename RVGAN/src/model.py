...
#from src.losses import weighted_feature_matching_loss
...
def coarse_generator(img_shape=(256, 256, 3),mask_shape=(256,256,1),ncf=64, n_downsampling=2, n_blocks=9, n_channels=1):
    ...
    model.compile(loss=['mse',None], optimizer=Adam(learning_rate=0.0002, beta_1=0.5, beta_2=0.999))

    model.summary()
    print("Input shapes and data types:")
    print("Shape of X_input: ", X_input.shape)
    print("Data type of X_input: ", X_input.dtype)
    print("Shape of X_mask: ", X_mask.shape)
    print("Data type of X_mask: ", X_mask.dtype)
...
def fine_generator(x_coarse_shape=(256,256,64),input_shape=(512, 512, 3), mask_shape=(512,512,1), nff=64, n_blocks=3, n_coarse_gen=1,n_channels = 1):
    ...
    print("Shape of X_coarse: ", X_coarse.shape)
    print("Data type of X_coarse: ", X_coarse.dtype)
    ...
    model.compile(loss='mse', optimizer=Adam(learning_rate=0.0002, beta_1=0.5, beta_2=0.999))
...
def discriminator_ae(input_shape_fundus=(512, 512, 3),
    ...
    model.summary()
    model.compile(loss=['mse',None,None,None,None,None,None], optimizer=Adam(learning_rate=0.0002, beta_1=0.5, beta_2=0.999))
    return model

def fm1(y_true, fake_samples, in_fine, label_fine, d_model1, inner_weight):
    return weighted_feature_matching_loss(y_true, fake_samples, in_fine, label_fine, d_model1, inner_weight)

def fm2(y_true, fake_samples, in_coarse, label_coarse, d_model2, inner_weight):
    return weighted_feature_matching_loss(y_true, fake_samples, in_coarse, label_coarse, d_model2, inner_weight)
...
def RVgan(g_model_fine, g_model_coarse, d_model1, d_model2, image_shape_fine, image_shape_coarse, image_shape_x_coarse, mask_shape_fine, mask_shape_coarse, label_shape_fine, label_shape_coarse, inner_weight):
    ...
    fm1_loss = partial(fm1, in_fine=in_fine, label_fine=label_fine, d_model1=d_model1, inner_weight=inner_weight)
    fm2_loss = partial(fm2, in_coarse=in_coarse, label_coarse=label_coarse, d_model2=d_model2, inner_weight=inner_weight)

    model = Model(inputs=[in_fine, in_coarse, in_x_coarse, in_fine_mask, in_coarse_mask, label_fine, label_coarse],
                  outputs=[dis_out_1_fake[0], dis_out_2_fake[0], gen_out_fine, gen_out_coarse, gen_out_coarse, gen_out_fine, gen_out_coarse, gen_out_fine])

    opt = tf.keras.optimizers.Adam(learning_rate=0.0002, beta_1=0.5)
    model.compile(loss=['hinge', 'hinge', fm1_loss, fm2_loss, 'hinge', 'hinge', 'mse', 'mse'],
                  optimizer=opt,
